        ...
        draw : function() {
            ...
            style && style.label 
                && (edge.label && edge.label.attr({x:(x1+x4)/2, y:(y1+y4)/2}) 
                    || (edge.label = selfRef.text((x1+x4)/2, (y1+y4)/2, style.label).attr({fill: "#000", "font-size": style["font-size"] || "18px"})));
    ...
    drawNode: function(node) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var ellipse = r.ellipse(0, 0, 30, 30).attr({fill: color, stroke: color, "stroke-width": 2});
                /* set DOM node ID */
